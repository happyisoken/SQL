...
# We believe that at this stage of the course, you've already covered a sufficient amount of relational database theory and MySQL tools to discover and understand a new MySQL tool or technique by just reading code with comments. The latter is a crucial skill for any database administrator, analyst, or business intelligence analyst in the professional world involving the use of SQL.

#And now is a great time to present you with a suitable challenge.

# We've provided you with an *.sql file containing code and comments about MySQL Triggers. 
# Try to learn this new material by using the *.sql file only. For your convenience, there is also a relevant video lecture right after that.

-- LECTURE: MySQL Triggers

# In this lesson, we will introduce you to MySQL triggers.

/* MySQL trigger is a type of stored program, associated with a table, that will be activated automatically once a specific event related to the table of association occurs. 
...
# UPDATE, or DELETE statement has been executed.
 For instance, a trigger can be activated before a new record is inserted into a table, or after a record has been updated.
...
/* Then, execute a COMMIT statement, because the triggers we are about to create will make some changes to the state of the data in our database. 
At the end of the exercise, we will ROLLBACK up to the moment of this COMMIT*/  
...
- First, especially for those of you who are familiar with some programming, this is an example of a conditional. 
The IF statement starts the conditional block. Then, if the condition for negative salary is satisfied, one must use the keyword THEN before showing 
...
- The third part of the syntax regards the SET keyword. As you already know, it is used whenever a value has to be assigned to a certain variable.
 Here, the variable is the newly inserted salary, and the value to be assigned is 0. 
...
CREATE TRIGGER before_salaries_insert -- we create trigger and specify name
BEFORE INSERT ON salaries			  -- we specify name of trigger and table we want it applied.
									  -- create a before trigger that would be activated whenever new data
                                      -- is inserted into the salaries table.
FOR EACH ROW	-- this statement designates that MySQL would perform a check for
				-- change in the state of the data on all rows affected by the trigger activation.
BEGIN 
	IF NEW.salary < 0 THEN 
		SET NEW.salary = 0; 
	END IF; 
...
BEFORE UPDATE ON salaries
  ...
  - Second, in the IF conditional statement, instead of setting the new value to 0, we are basically telling MySQL to keep the old value. 
	Technically, this is achieved by setting the NEW value in the “Salary” column to be equal to the OLD one. 
    This is a good example of when the OLD keyword needs to be used.
/*
	IF NEW.salary < 0 THEN 
		SET NEW.salary = OLD.salary; 
	END IF;
...
All right. For the moment, you know you have created only two triggers. But how could you prove that to someone who is seeing your script for the first time?
...
Let’s introduce you to another interesting fact about MySQL. You already know there are pre-defined system variables, but system functions exist too! 
...
For instance, the following query could extract the current date, quoting the year, the month, and the day */
...
/* Conceptually, this was an ‘after’ trigger that automatically added $20,000 to the salary of the employee who was just promoted as a manager. 
  Moreover, it sets the start date of her new contract to be the day on which you executed the insert statement.
...
Values('999999', '1984-01-01', 'Isoken', 'Igbinovia', 'F', '2022-08-04');
